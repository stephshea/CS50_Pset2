...
int main(int argc, string argv[]) // run program, get number for cipher key, initially stored as string
{
    ...
    for (i = 0; i < strlen(cipherText); i++) // iterate through each char in plaintext string
    {
        ...
        else if (isalpha(cipherText[i])) //check to see if each char is alpha
        {
            ...
            {
                ciphe = ((cipherText[i] - 'A' + key[i]%klength) % 26) + 'A'; // convert upperChars with key shift to ascii to alpha to ascii

            }
            else if (islower(cipherText[i]))
            {
                ciphe = ((cipherText[i] - 'a' + key[i]&klength) % 26) + 'a'; //converts lowerChars with key shift to ascii to alpha to ascii
